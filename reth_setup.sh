...
echo -e "alias lh='lighthouse bn     --checkpoint-sync-url https://mainnet.checkpoint.sigp.io     --execution-endpoint http://localhost:8551     --execution-jwt $TARGET_DIR/jwt.hex   --disable-deposit-contract-sync'" >> ~/.bashrc
echo -e "alias r='reth node --datadir $TARGET_DIR/reth/data/ --metrics 127.0.0.1:9001 --http --http.addr 0.0.0.0 --http.api All --ws  --authrpc.jwtsecret $TARGET_DIR/jwt.hex'" >> ~/.bashrc
echo -e "alias both='tmux new-session -d -s lh bash -i -c \"lh\"; tmux new-session -d -s r bash -i -c \"r\"'" >> ~/.bashrc
echo -e "both" >> ~/.bashrc
source ~/.bashrc
